...
Locally modified by Seventwos: installation commands are pinned to the centrally approved graphifyy 0.9.63. Do not install another version or upgrade to latest. Verify `graphify --version` and the installed distribution's Apache-2.0 licence before updates; retain `LICENSE`, `NOTICE`, and `LICENSE-MIT` when refreshing this skill. Generated `graphify-out/` content must remain untracked.
...
if [ -z "$PYTHON" ] && command -v uv >/dev/null 2>&1; then
    _UV_PY=$(uv tool run --from graphifyy==0.9.63 python -c "import sys; print(sys.executable)" 2>/dev/null)
...
if ! "$PYTHON" -c "import graphify; from importlib.metadata import version; assert version('graphifyy') == '0.9.63'" 2>/dev/null; then
    if command -v uv >/dev/null 2>&1; then
        uv tool install graphifyy==0.9.63 -q 2>&1 | tail -3
        _UV_PY=$(uv tool run --from graphifyy==0.9.63 python -c "import sys; print(sys.executable)" 2>/dev/null)
        if [ -n "$_UV_PY" ]; then PYTHON="$_UV_PY"; fi
    else
        "$PYTHON" -m pip install graphifyy==0.9.63 -q 2>/dev/null \
          || "$PYTHON" -m pip install graphifyy==0.9.63 -q --break-system-packages 2>&1 | tail -3
    fi
fi
...
> Tip: set `GEMINI_API_KEY` or `GOOGLE_API_KEY` to use Gemini for semantic extraction (`pip install 'graphifyy[gemini]==0.9.63'`).
